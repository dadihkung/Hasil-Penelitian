...
def parse_args():
    ...
    p.add_argument("--baseline", action="store_true",
                   help="Run baseline recording (blank/gray screen) instead of visual flicker.")
    return p.parse_args()
...
class BlinkApp:
    ...
    def run_baseline(self, cyton: CytonSession, duration=10):
        start_time = time.perf_counter()
        cyton.insert_marker(10.0)  # baseline start
        while (time.perf_counter() - start_time) < duration and self.running:
            self.screen.fill((128, 128, 128))  # gray background
            pygame.display.flip()
            self.clock.tick(60)
        cyton.insert_marker(20.0)  # baseline end
...
def main():
    ...
    if args.outfile:
        ...
        out_path = Path(r"C:\laragon\www\skripsi\data_mentah_brainflow") / f"martin6_recording_{ts}.csv"
        out_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        if args.baseline:
            app.run_baseline(cyton, duration=args.duration)
            stim_wall_start, stim_wall_end = None, None  # no flicker, just rest
        else:
            app.phase = "stim"
